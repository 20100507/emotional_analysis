...
public class ForkJoinCalculate extends RecursiveTask<Long>{
	...
	public ForkJoinCalculate(long start, long pageSize,long songId) {
		this.start = start;
	...
	protected Long compute() {
		long length = pageSize - start;
		if(length <= THRESHOLD){
			for (long i = start; i <= pageSize; i++) {
				if(i % 2 == 0){
					try{
...
//						pageSize = total / 10;
						...
						for (Comments comments2 : comments) {
					        ...
					        pstmt.executeUpdate();
					        System.out.println("当前线程为:"+Thread.currentThread().getName()+"==========================================="+(j++)+" 昵称:"+nickname+"\r评论内容为："+content+"\r评论时间为:"+time+"\r头像地址"+avatarUrl);
						}
					}catch(Exception e){
					}
				}else{
...
//						pageSize = total / 10;
			...
			long middle = (start + pageSize) / 2;
			ForkJoinCalculate left = new ForkJoinCalculate(start, middle,songId);
			ForkJoinCalculate right = new ForkJoinCalculate(middle+1, pageSize,songId);
